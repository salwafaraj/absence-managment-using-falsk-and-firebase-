...
cred = credentials.Certificate("./misc/serviceAccountKey.json")
firebase_admin.initialize_app(
    cred,
    {
        "databaseURL": "https://system-for-student-attendance-default-rtdb.firebaseio.com/",
        "storageBucket": "system-for-student-attendance.appspot.com",
    },
)
...
def generate_frame():
    ...
    file = open("./misc/EncodeFile.p", "rb")
...
#######################Login Admin2###############################################################################
@app.route("/login_admin222", methods=["GET", "POST"])
def admin_login2():
    id = request.form.get("id_number", False)
    email = request.form.get("email", False)
    password = request.form.get("password", False)
    ...
    if id:
        if id not in studentIDs:
            return render_template(
                "login_admin.html", data=" ❌ The id is not registered"
            )
        else:
            if (
                dataset(id)[0]["password"] == password
                ...
                id = False
                return render_template(
                    "login_admin.html", data=" ❌ Email/Password Incorrect"
                )
    else:
...
def add_user():
    ...
    file = open("./misc/EncodeFile.p", "wb")
...
def delete_image(student_id):
    filepath = f"Images/{student_id}.jpg"
